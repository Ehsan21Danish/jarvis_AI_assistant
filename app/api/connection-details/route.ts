...
function getConfig() {
  const pathsToTry = [
    ...
    path.join(process.env.USERPROFILE || '', 'Desktop', 'Jarvis', 'user_config.json') // Optional Windows fallback
  ];
...
export async function POST(req: Request) {
  try {
    ...
    const agentName: string = body?.room_config?.agents?.[0]?.agent_name || 'jarvis';
    console.log("Debug: Dispatching LiveKit agent:", agentName);
