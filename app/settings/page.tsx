...
export default function SettingsPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        search_engine_id: '',
        openweather_key: '',
        system_control_permission: false,
            ...
            try {
                ...
                if (data.exists) {
                    setFormData({
                        ...
                        llm_model: data.llm?.model || 'gemini-2.5-flash-native-audio-latest',
                        ...
                        system_control_permission: Boolean(data.permissions?.system_control),
                        whatsapp_permission: Boolean(data.permissions?.whatsapp),
                    });
                } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const configData = {
                ...
                permissions: {
                    system_control: formData.system_control_permission,
                    whatsapp: formData.whatsapp_permission,
                }
            };
        ...
        <div className="min-h-screen bg-black text-white font-mono selection:bg-cyan-500/30 overflow-x-hidden relative">
            ...
            <div className="relative z-10 container mx-auto px-4 py-8 max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    {/* Owner Permissions */}
                    <div className="relative p-6 rounded-xl border border-red-500/30 bg-slate-950/50 backdrop-blur-md shadow-[0_0_15px_rgba(239,68,68,0.05)]">
                        <div className="absolute top-0 right-0 p-2 text-[10px] text-red-800 font-bold tracking-widest opacity-60">OWNER_PERMISSION</div>
                        <label className="flex items-start gap-3 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={formData.system_control_permission}
                                onChange={(e) => setFormData({ ...formData, system_control_permission: e.target.checked })}
                                className="mt-1 h-4 w-4 accent-cyan-500"
                            />
                            <span>
                                <span className="block text-cyan-100 uppercase text-xs tracking-wider">Allow System Control</span>
                                <span className="mt-1 block text-xs text-cyan-700/90 leading-relaxed">
                                    Allows Jarvis to open applications and browser tabs from voice commands. Arbitrary terminal commands are not enabled.
                                </span>
                            </span>
                        </label>
                        <label className="mt-4 flex items-start gap-3 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={formData.whatsapp_permission}
                                onChange={(e) => setFormData({ ...formData, whatsapp_permission: e.target.checked })}
                                className="mt-1 h-4 w-4 accent-cyan-500"
                            />
                            <span>
                                <span className="block text-cyan-100 uppercase text-xs tracking-wider">Allow WhatsApp Messaging</span>
                                <span className="mt-1 block text-xs text-cyan-700/90 leading-relaxed">
                                    Allows Jarvis to open WhatsApp Web and send messages only when the owner gives a clear command.
                                </span>
                            </span>
                        </label>
                    </div>
